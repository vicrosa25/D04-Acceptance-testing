...
@Transactional
public class editSocialProfileTest extends AbstractTest {
	...
	protected void template(Class<?> expected, String actor, String socialProfile, String nick) {
		...
		try {
			...
			saved = this.socialProficeServcie.save(socialIdentity);

			
			// Check.
			Assert.isTrue(socialIdentity.getId() == saved.getId());
